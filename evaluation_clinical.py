...
def main():
    ...
    # Initialize model
    model = BERTClassifier('medicalai/ClinicalBERT').to(device)
    
    # Load the saved model weights
    try:
        model.load_state_dict(torch.load('clinical_best_model.pt', map_location=device))
        print("Successfully loaded model from 'clinical_best_model.pt'")
    except Exception as e:
        print(f"Error loading model: {e}")
        return
    
    # Evaluate model
    print("Evaluating model on 20% of test data...")
    results, predictions, true_labels, y_true_flat, y_pred_flat = evaluate_model(model, test_dataloader, device)
    
    # Print results
    print("\n===== Model Evaluation Results (20% of Test Data) =====")
    print("\nPer-class metrics:")
    ...
    with open('evaluation_results_20percent.txt', 'w') as f:
    ...
    print("\nEvaluation results saved to 'evaluation_results_20percent.txt'")
